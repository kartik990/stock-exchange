...
echo "▶️ Starting backend services (api, ws, engine, mm, db-service)..."
pm2 start ecosystem.config.js --only api,ws,engine --wait-ready

echo "▶️ Starting backend services (api, ws, engine, mm, db-service)..."
pm2 start ecosystem.config.js --only mm,db-service --wait-ready
