...
export class Orderbook {
  ...
  addOrder(order: Order): {
    ...
    if (order.side === "buy") {
      ...
      const maxPrice = fills.reduce((acc, f) => {
        if (+f.price > acc) return +f.price;
        return acc;
      }, 0);
      ...
      const minPrice = fills?.reduce((acc, f) => {
        if (+f.price < acc) return +f.price;
        return acc;
      }, 0);
  ...
  matchBid(order: Order): { fills: Fill[]; executedQty: number } {
    ...
    if (executedQty > 0) this.lastTradeKind = "buy";
  ...
  matchAsk(order: Order): { fills: Fill[]; executedQty: number } {
    ...
    if (executedQty > 0) this.lastTradeKind = "sell";
